...
class DB:
    ...
    def addTransaction(self, query, values):
        connection = self.connect()
        try:
            cursor = connection.cursor()
            if values:
                bob = cursor.mogrify(query, values)
            else:
               bob = cursor.mogrify(query)
    ...
    def get_queries_from(self, sql_file, delimiter=";"):
        ...
        while True:
            ...
            if test is "":
                ...
                continue
            elif test.find("DELIMITER") != -1:
                #real.append(test[0:len(test)-1])
                if count == False:
                    delimiter = "$"
                    count = True
                else:
            ...
            elif test[len(test)-2] == delimiter:
                real.append(test[0:len(test)-2])
            else:
                while True:
                    holder = file.readline()
                    if holder[len(holder)-2] == delimiter:
                        test += holder
                        break
                    else:
                        test += holder
                if delimiter == "$":
                    real.append(test[0:len(test)-3])
                else:
                    real.append(test[0:len(test)-2])
        return real
    ...
    def insert(self, query=None, table=None, attributes=None, values=None):
        if not query:
            ...
            values = tuple(values)
            self._execute_query(query, values, action=self.INSERT)
        return self.addTransaction(query, values)

    def update(self, query, values=None):
        self._execute_query(query, values, action=self.UPDATE)
        return self.addTransaction(query, values)

    def delete(self, query, values=None):
        self._execute_query(query, values, action=self.DELETE)
        return self.addTransaction(query, values)
